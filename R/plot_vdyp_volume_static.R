#' Plot Merch Vol maps from VDYP yield curve data
#'
...
#' the `match_vri_to_vdyp_curves()` function, which prepares the
#' data in the required format.
#'
#' @param vri_vdyp_matched An `sf` data frame, typically the `vri_vdyp_matched`
#'   element from the list returned by `match_vri_to_vdyp_curves()`.
...
#' }
#' @export
plot_vdyp_volume_static <- function(vri_vdyp_matched, age_to_plot = 80) {

  # Initialize a list to hold the plots
  plot_list <- list()

  # Construct dynamic column names for plotting
  conifer_col <- paste0("Conifer_Vol_CU_Age", age_to_plot)
  deciduous_col <- paste0("Decid_Vol_CU_Age", age_to_plot)

  # Ensure the required columns exist
  if (!all(c(conifer_col, deciduous_col) %in% names(vri_vdyp_matched))) {
    stop(paste0("The 'vri_vdyp_matched' data frame does not contain the required columns for age ",
                age_to_plot, ". Please check your data."))
  }

  # Map 1: Conifer volume
  p1 <- ggplot2::ggplot(vri_vdyp_matched) +
  ...
  p2 <- ggplot2::ggplot(vri_vdyp_matched) +
